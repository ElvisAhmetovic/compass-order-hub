...
const buildOrderCreatedEmailHtml = (data: {
  clientName: string;
  clientEmail: string;
  clientPhone: string;
  companyName: string;
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Order Confirmation - AB Media Team</title></head>
      ...
      <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="600" style="max-width:600px; background-color:#ffffff;">
        ...
        <tr><td style="padding:22px 32px;">
          <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%"><tr>
            <td align="left" style="font-family:Roboto,Arial,sans-serif; font-size:13px; color:#606368;">Order Confirmation</td>
        ...
        <tr><td style="padding:16px 32px 24px; text-align:center;">
          <h1 style="margin:0; font-family:Roboto,Arial,sans-serif; font-size:28px; color:#606368; font-weight:normal; line-height:1.3;">Your order has been successfully created!</h1>
        </td></tr>
          ...
          <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color:#f8f9fa; border:1px solid #dadce0; border-radius:12px; padding:24px;">
            <tr>
              ...
              <td style="padding-left:16px; vertical-align:top;">
                <div style="font-weight:bold; color:#0f1b3d; font-size:16px;">${data.companyName}</div>
                ...
                <div style="color:#1e3a5f; font-size:18px; font-weight:bold; margin-top:8px;">Price: ${formattedPrice}</div>
        ...
        <tr><td style="text-align:center; padding:16px 32px 32px;">
          <a href="${data.portalUrl}" style="display:inline-block; height:48px; padding:0 28px; border-radius:8px; background:#1e3a5f; color:#ffffff; font-family:Roboto,Arial,sans-serif; font-size:16px; font-weight:700; line-height:48px; text-decoration:none; white-space:nowrap; box-shadow:0 1px 2px rgba(60,64,67,.15),0 2px 6px rgba(60,64,67,.10);">Go to Client Portal</a>
        </td></tr>
        ...
        <tr><td style="padding:8px 32px 16px; font-family:Roboto,Arial,sans-serif; font-size:14px; color:#606368; line-height:1.7; text-align:center;">
          Thank you for your order. We are excited to get started on your project! You can track progress, view invoices, and communicate with our team through your Client Portal.<br><br>
          Click the button above to access your portal. If you need login credentials, you'll be able to request them directly from the portal page.
        </td></tr>
...
serve(async (req) => {
  ...
  try {
    ...
    const { clientEmail, clientName, clientPhone, companyName, description, price, currency, orderId } = body;
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [clientEmail],
        subject: `Order Confirmed – ${companyName} | AB Media Team`,
        html,
      }),
    });
    ...
    console.log('Order created notification sent to:', clientEmail);

    return new Response(JSON.stringify({ success: true, id: result.id }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error: any) {
    console.error('Error in send-order-created-notification:', error);
