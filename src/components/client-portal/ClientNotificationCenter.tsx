...
const ClientNotificationCenter = () => {
  ...
  const isClientRelevant = (n: { action_url?: string | null }) =>
    n.action_url?.startsWith('/client/');

  useEffect(() => {
    if (!user) return;

    const fetchNotifications = async () => {
      const data = await NotificationService.getClientNotifications(user.id);
      setNotifications(data);
    };

    fetchNotifications();

    const handleChanged = () => fetchNotifications();
